...
const FeedbackSchema = z.object({
  ...
  freshness_rating: z.number().int().min(1).max(5),
  reflection: z.string().trim().max(2000),
  private_note: z.string().max(4000).optional(),
  source_path: z.string().trim().max(500).optional(),
  output_reference: z.string().trim().max(300).optional(),
});
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data, error } = await supabase.from('feedback_sessions').select('id,module,accuracy_rating,freshness_rating,reflection,private_note,source_path,output_reference,curation_status,created_at').eq('user_id', user.id).order('created_at', { ascending: false }).limit(50);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data, error } = await supabase.from('feedback_sessions').insert({ ...body, user_id: user.id }).select('id,module,accuracy_rating,freshness_rating,reflection,private_note,source_path,output_reference,curation_status,created_at').single();
    if (error) throw error;
