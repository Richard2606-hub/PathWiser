...
import { UNIVERSITIES } from '@/lib/corpus/universities';
...
export function CurriculumEngineView() {
  ...
  const [institution, setInstitution] = useState('Universiti Malaya (UM)');
  ...
  const analyse = async (event?: FormEvent, override?: { programme: string; destination: string; currentSkills: string }) => {
    ...
    try {
      ...
      const result = await navigate({ userId: 'anon', persona: 'university', role: effectiveDestination, education: `${institution} · ${effectiveProgramme}`, years_experience: 1, state: 'Kuala Lumpur', skills, life_stage: 'young_adult' });
  ...
  useEffect(() => {
    void loadWorkspaceRecords<{
      institution: string; programme: string; destination: string; currentSkills: string; recommendations: Recommendation[]; cohortSize: number; source: string; leadTime: number;
  ...
  const exportRecommendations = () => {
    const rows = [['institution','programme','destination','skill','cohort_prevalence_percent','recommended_action','trade_off','evidence_source'], ...recommendations.map((item) => [institution,programme,destination,item.skill,Math.round(item.prevalence * 100),item.action,item.tradeOff,source])];
  ...
  const saveFacultyReview = async () => {
    ...
    const saved = await saveWorkspaceRecord({
      ...
      title: `${institution} · ${programme} · ${destination}`,
      status: 'review_due',
      payload: { institution, programme, destination, currentSkills, recommendations, cohortSize, source, leadTime },
  ...
  return <div className="flex flex-col gap-4">
    ...
    <form onSubmit={analyse} className="grid gap-2 rounded-md border border-[color:var(--border)] bg-[color:var(--bg-glass)] p-4 md:grid-cols-2">
      <label className="text-xs">Institution / University
        <select className="community-input mt-1" value={institution} onChange={(e) => setInstitution(e.target.value)}>
          {UNIVERSITIES.map((u) => (
            <option key={u.id} value={u.name}>{u.name} ({u.type})</option>
          ))}
        </select>
      </label>
      <label className="text-xs">Programme<input className="community-input mt-1" value={programme} onChange={(event) => setProgramme(event.target.value)} required /></label>
      <label className="text-xs">Target graduate destination<input className="community-input mt-1" value={destination} onChange={(event) => setDestination(event.target.value)} required /></label>
      <label className="text-xs">Expected approval lead time<select className="community-input mt-1" value={leadTime} onChange={(event) => setLeadTime(Number(event.target.value))}><option value={1}>1 semester</option><option value={2}>2 semesters</option><option value={3}>3 semesters</option></select></label>
      <label className="text-xs md:col-span-2">Current curriculum skills<input className="community-input mt-1" value={currentSkills} onChange={(event) => setCurrentSkills(event.target.value)} required /></label>
      <Button className="self-end justify-self-start md:col-span-2" disabled={loading}>{loading ? 'Analysing…' : 'Refresh gap analysis'}</Button>
    </form>
