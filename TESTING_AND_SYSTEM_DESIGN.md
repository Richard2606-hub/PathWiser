...
| Area | Check | Result |
|---|---|---|
| Engine math, normalization, ranking, salary presentation, coach honesty and workspace continuity | Percentiles, distributions, MyCOL flags, salary ranges, bridges, small-cohort guard, probability totals, taxonomy handling, explainable marketplace ranking, whole-ringgit presentation, predictive-language rejection, transient-provider classification, deterministic fallback, and device fallback for the six Final Kit workspace modules | Pass: 47 automated Vitest tests on 25 July 2026. |
...
| Production account and RLS isolation | Two temporary confirmed users, own-profile reads, cross-account update, employer membership, employer-only RPC, explicit discovery consent, identifier privacy, immediate revocation and cleanup | Pass: 9/9 against live Supabase on 24 July 2026. Temporary users, organisation and consent/profile records were removed and cleanup was verified. |
...
1. `npm.cmd run lint`, `npm.cmd run test`, `npm.cmd run typecheck`, `npm.cmd run test:openapi`, `npm.cmd run build`, `npm.cmd run test:smoke`, `npm.cmd run test:a11y`, and `npm.cmd audit --omit=dev --json` pass.
