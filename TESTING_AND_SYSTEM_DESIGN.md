...
| Release preflight | Non-secret migration, applied DB checksum, environment and optional target health readiness check | Added `npm.cmd run release:preflight`; it fails on missing migration files or checksum mismatches, verifies applied DB migration checksums when `DATABASE_URL` or `SUPABASE_DB_URL` is present, warns on owner-controlled full-launch settings in preview/dev environments, treats warnings as release-blocking when `RELEASE_PREFLIGHT_STRICT=true`, and now runs in the GitHub Production quality gate. |
