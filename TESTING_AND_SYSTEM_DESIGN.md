...
| Repeatable hard-testing harness | Multi-iteration production smoke and accessibility cycles against any configured `PATHWISER_BASE_URL` | Pass: default local run completed 3 iterations / 6 checks on 25 July 2026. Use `HARD_TEST_MINUTES=120` for the requested two-hour soak against the final deployment. |
